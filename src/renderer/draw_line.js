...
export default function drawLine(dataInRenderSpace, {
    color, width=1, context, shadowColor='black', shadowBlur=5, dashed=false, dashPattern=null, highlighted=false, showIndividualPoints=false, pointRadius, pointShape, pointColor: pointColorOption, minPointSpacing, getIndividualPoints, getRanges, cutoffIndex, cutoffTimeValue, cutoffOpacity, originalData, renderCutoffGradient, selectionBounds, rendering, isPreview, negativeColor, hasNegatives, zero, zeroColor, roundedLines=false
}) {
    if (!context) {
            ...
            if (cutoffTime !== null) {
                ...
                if (timeRatio > 1) {
                    ...
                    for (let i = 0; i < spacedPoints.length; i++) {
                        ...
                        pointColor = resolvePointColor(pointColorOption, spacedPoints[i]) || pointColor;
                        ...
                        context.beginPath();
                        tracePointPath(context, resolvePointShape(pointShape, spacedPoints[i]), x, y, pointRadius || 8);
                        context.fill();
                    }
                    return;
                }
